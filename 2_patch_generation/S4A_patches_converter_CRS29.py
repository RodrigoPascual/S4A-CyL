...
# Maximum allowed zero percentage threshold in the B02 band
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Generates NetCDF patches from Sentinel, parcels, and labels.")
    ...
    parser.add_argument("--base_dir", default="Datos brutos 2023",
                        help="Base path where Sentinel, Parcels, and Labels folders are located.")
    args = parser.parse_args()
    return args

def find_safe_dirs(sentinel_folder):
    """
    Searches for .SAFE subdirectories in the 'sentinel_folder'.
...
def load_band_path(safe_path, band_name):
    ...
    returns the path to the corresponding JP2 file (or None if it doesn't exist).
    """
...
def compute_coords_bands(raster_path, x_i, y_i, band_name):
    """
    Calculates 1D arrays (y_vals, x_vals) for the 'band_name' band
...
def read_patch_same_area(ref_raster_path, x_i, y_i, ref_band, target_raster_path):
    ...
    that corresponds to the patch (x_i,y_i) in the 'ref_band' of 'ref_raster_path'.

    1) geospatial bounding box in ref_raster
    2) inverse transform in target_raster
    3) read with Window(...) => crops/overwrites to (366x366) if the reference patch is B02.
    """
...
def read_time_series_of_patch_B02(safe_list, x_i, y_i):
    ...
    If no B02 file exists, it returns None.
    """
    band_name = "B02"
    b02_data_list = []
...
def reproject_raster(input_path, output_path, target_crs):
    """
    Reprojects a given raster 'input_path' to the 'target_crs' CRS
...
def main():
    ...
    for year in args.years:
        for tile_name in args.tiles:
            ...
            if not safe_list:
                print(f"No .SAFE subfolders found in {sentinel_dir}")
                continue
            ...
            if not ref_10m_path:
                print(f"No .SAFE folder with B02 band in {sentinel_dir}")
                continue


            with rasterio.open(ref_10m_path) as src:
                sentinel_crs = src.crs
            print(f"Sentinel reference CRS: {sentinel_crs}")


            if sentinel_crs.to_epsg() is not None and str(sentinel_crs.to_epsg()).startswith("32629"):
                print("Sentinel tile with EPSG:32629 projection detected. "
                      "Verifying parcels and labels rasters for reprojection...")

                parcels_reproj_path = base_dir / "Parcelas CyL" / f"parcels{year}_32629.tif"
                labels_reproj_path  = base_dir / "Mapa ITACYL cultivos" / f"labels{year}_32629.tif"

                with rasterio.open(parcels_raster) as src:
                    parcels_crs = src.crs
                if parcels_crs.to_epsg() != sentinel_crs.to_epsg():
                    if not parcels_reproj_path.exists():
                        print(f"Parcels raster is in EPSG:{parcels_crs.to_epsg()}, "
                              f"reprojecting to EPSG:{sentinel_crs.to_epsg()}...")
                        reproject_raster(str(parcels_raster), str(parcels_reproj_path),
                                         target_crs=f"EPSG:{sentinel_crs.to_epsg()}")
                        print(f"Reprojection completed for parcels. Saved to {parcels_reproj_path}.")
                    else:
                        print(f"Reprojected parcels raster already exists at {parcels_reproj_path}.")
                    parcels_raster = parcels_reproj_path
                else:
                    print("The parcels raster already has the correct projection.")

                with rasterio.open(labels_raster) as src:
                    labels_crs = src.crs
                if labels_crs.to_epsg() != sentinel_crs.to_epsg():
                    if not labels_reproj_path.exists():
                        print(f"Labels raster is in EPSG:{labels_crs.to_epsg()}, "
                              f"reprojecting to EPSG:{sentinel_crs.to_epsg()}...")
                        reproject_raster(str(labels_raster), str(labels_reproj_path),
                                         target_crs=f"EPSG:{sentinel_crs.to_epsg()}")
                        print(f"Reprojection completed for labels. Saved to {labels_reproj_path}.")
                    else:
                        print(f"Reprojected labels raster already exists at {labels_reproj_path}.")
                    labels_raster = labels_reproj_path
                else:
                    print("The labels raster already has the correct projection.")

                print("Parcels and labels rasters with matching projection found. "
                      "Continuing with patch creation.")
            else:
                print("Sentinel tile projection is not EPSG:32629. "
                      "Reprojection of parcels and labels is not required.")
            ...
            for x_i in range(NX_PATCHES):
                for y_i in range(NY_PATCHES):
                    ...
                    # Calculate fraction of zeros in the entire B02 time series
                    # b02_stack.shape => (n_dates, 366, 366)
                    ...
                    for band_name, (subfolder, res_m) in BANDS_INFO.items():
                        ...
                        # If resolution is 10m and we don't have coords_10m yet
